...
```{r}
fancy <- as(items(sort(mc_itemsets, by="support")), "matrix")
labeled <- df %>% rowwise() %>%
  mutate(CLASS = paste(
    which(apply((matrix(!is.na(c_across(-(1:7))), nrow=nrow(fancy), ncol=ncol(fancy), byrow=TRUE, dimnames=dimnames(fancy)) & fancy) == fancy, 1, all)), collapse=""), .after=OUTCOME ) %>%
...
```
```{r test one liner, echo=FALSE}
paste(which(apply((matrix(!is.na(df[4,-(1:7)]), nrow=nrow(fancy), ncol=ncol(fancy), byrow=TRUE, dimnames=dimnames(fancy)) & fancy)==fancy, 1, all)), collapse="")
```
```{r, fig.width=10, fig.height=8}
group.plot <- ggplot(
  labeled %>% slice_sample(n=1000), 
