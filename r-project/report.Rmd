...
class_labels[1] <- "CBC"
class_labels[3] <- "Group3"
class_labels[10] <- "Group10"
```
...
paste(which(
  apply((matrix(!is.na(df[4,-(1:7)]), nrow=nrow(classes), ncol=ncol(classes), byrow=TRUE, dimnames=dimnames(classes)) & classes)==classes, 1, all)
  ), collapse="")
```
...
group.plot <- ggplot(
  ...
  geom_rug(sides="b", outside=TRUE, mapping = aes(x=ADMISSION_TIME, color=NULL)) + coord_cartesian(clip="off")
