...
days.to.outcome %>% summarise(DAYS_TO_OUTCOME = quantile(DAYS_TO_OUTCOME, c(0, 0.25, 0.5, 0.75,1)), quantile = c("min", "1st", "2nd", "3rd", "max"))

```

```{r, eval=FALSE}
ggplot(days.to.outcome, aes(DAYS_TO_OUTCOME)) + geom_histogram(binwidth = 1)
```
...
ht %>% 
  group_by(OUTCOME) %>%
  summarise(
    HOSPITALIZATION_TIME = quantile(HOSPITALIZATION_TIME, c(0, 0.25, 0.5, 0.75,1)), 
    stat = c("min", "q1", "median", "q3", "max"), 
    .groups="drop"
  ) %>%
  pivot_wider(names_from=OUTCOME, values_from=HOSPITALIZATION_TIME)
)
```
