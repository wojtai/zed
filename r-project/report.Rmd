...
mamy określić który pacjent przeżyje, więc dane zostaną zredukowane tylko do pacjentów, pozostawiając ostatnią wartość badania

wcześniej, ze zbioru zostaną usunięte te wyniki badań, które zostały uzyskane przed wypisem pacjenta ze szpitala
...
```{r}
fs.control <- trainControl(method="repeatedcv", number=10, repeats=3)
fs.model <- train(OUTCOME~., 
                  data=patients, 
                  method="rf", 
                  trControl=fs.control, 
                  mtree=30
                  )
```
