...
```{r row counts patient data frame}
patients_with_number_of_tests <- data.frame(PATIENT_ID = gb.patient_id$PATIENT_ID, number_of_tests = rc.gb.patient_id)
total_tests_count <- ncol(df) - 7
threshold <- 0.65
patients_to_be_removed <-patients_with_number_of_tests %>% filter(number_of_tests < total_tests_count*threshold)
